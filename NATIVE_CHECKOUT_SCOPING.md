...
## Open verification held for Peter
- **FAMILY_MEAL settlement example (promo Path B):** the FAMILY_MEAL half of the empirical verification is **held open pending Peter providing a FAMILY_MEAL test restaurant/table.** DIRECT is fully verified to the cent; FAMILY_MEAL is amount-only (no transfer) and structurally a subset, but the live example is pending.
