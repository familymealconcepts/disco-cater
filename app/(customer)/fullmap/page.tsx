      ...
      <div key="results" style={{ padding: p }}>
        ...
        {treeResults.map(r => {
          ...
          <div key={r._id} style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #e8e8e8', overflow: 'hidden', marginBottom: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
            {r.image && (
              <img src={sizedImage(r.image, 400, 260)} alt={r.name} loading="lazy" decoding="async" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} style={{ width: '100%', height: cardImgH, objectFit: 'cover', display: 'block' }} />
            )}
                      ...
                      {filtered.map((r, i) => (
                        <div key={r._id} style={{ flexShrink: 0, width: 'calc(100% - 64px)', scrollSnapAlign: 'start', marginLeft: i === 0 ? 16 : 0, paddingTop: 12 }}>
                          ...
                          >
                            ...
                            />
                            <RestaurantCardImage src={r.image} w={320} h={192} alt={r.name} style={{ width: '100%', height: 96 }} />
