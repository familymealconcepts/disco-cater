...
const TESTS: TestDef[] = [
  ...
  { id: 'test-16', num: 16, name: 'Restaurant-Funded Promo Settlement (webhook reversal)', description: 'End-to-end restaurant-funded settlement through the real webhook path (processTransferReversal). DIRECT destination charge → plain customer refund → seed pending reversal → fire transfer.created → asserts the restaurant’s transfer is reversed by the FULL discount (raw Stripe records), then redelivers the event to prove no double reversal. Requires STRIPE_TEST_SECRET_KEY + STRIPE_TEST_CONNECTED_ACCOUNT; skips otherwise. Stripe TEST mode only.' },
]
