...
export default function AddNewMenuSettingsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d.error || 'Failed to create menu.')
        return
      }
      const data = await res.json()
      const ref = data.reference || data.id
      if (ref) {
        router.push(`/restaurant/manage-v2/${ref}`)
      } else {
        router.push('/restaurant/manage-v2/menus')
      }
    } catch {
  ...
  return (
    <div style={{ padding: '28px 32px', fontFamily: F, maxWidth: 640 }}>
      ...
      <h1 style={{ fontSize: 22, fontWeight: 700, color: DARK, margin: '0 0 28px' }}>Create Menu</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input
              ...
              placeholder="e.g. Summer Catering Menu"
              style={inputStyle}
              required
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              type="button"
              onClick={() => router.push('/restaurant/manage-v2/menus')}
              style={{
