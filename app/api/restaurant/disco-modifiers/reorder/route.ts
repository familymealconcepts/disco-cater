...
// Reorder the restaurant's disco_modifiers library. Body { references: string[] } is the
// full desired order; each row's position becomes its index.
//
// ADMIN-LIST ORDER ONLY. Neither disco_modifiers.position is read by any
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const moved = await applyOrder('disco_modifiers', scope, refs)
    return NextResponse.json({ success: true, requested: refs.length, moved })
  } catch (e) {
    console.error('[disco-modifiers/reorder] failed:', e instanceof Error ? e.message : e)
