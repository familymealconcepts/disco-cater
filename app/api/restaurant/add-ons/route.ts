...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${FM}/api/addOns?${params}`, { headers: h })
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${FM}/api/addOns`, { method: 'POST', headers: { ...h, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    if (!res.ok) return NextResponse.json({ error: 'Failed' }, { status: res.status })
