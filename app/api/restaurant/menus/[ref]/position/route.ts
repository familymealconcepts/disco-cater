...
// Mirrors FM menu.service.ts:66-68 —
//   PUT /api/menu/{reference}/position?position={position}  (null body)
export async function PUT(req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  ...
  try {
    const res = await fetch(`${FM}/api/menu/${ref}/position?position=${position}`, { method: 'PUT', headers: h })
