...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${FM}/api/closedDays/${ref}`, { method: 'PUT', headers: { ...h, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  ...
  try { h = await getRestaurantAuthHeader() } catch { return NextResponse.json({ error: 'Not authenticated' }, { status: 401 }) }
  try {
    const res = await fetch(`${FM}/api/closedDays/${ref}`, { method: 'DELETE', headers: h })
    if (!res.ok) return NextResponse.json({ error: 'Failed' }, { status: res.status })
