...
export async function GET() {
  ...
  try {
    const res = await fetch(`${FM}/api/closedDays`, { headers: h })
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${FM}/api/closedDays`, { method: 'POST', headers: { ...h, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    if (!res.ok) return NextResponse.json({ error: 'Failed' }, { status: res.status })
