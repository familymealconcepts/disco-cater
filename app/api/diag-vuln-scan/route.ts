...
// TEMPORARY diagnostic — for restaurants with online ordering ON, return those
// that would auto-disable on their next save (fail FM's address/contact/Stripe
// validation), with FM reference + whether they have a business/address phone
// (the copyable fix source). Chunked by page range. REMOVE after use.
export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'
export const maxDuration = 60

const FM = process.env.FM_API_BASE_URL || 'https://api.familymeal.com'
const TOKEN = '45533d9e21fda90bdba1caf8a1e1afeaaf69f2ed9019fdc6'
const SIZE = 200
...
export async function GET(req: NextRequest) {
  ...
  const checked = await mapPool(on, 8, async (r) => {
    ...
    const fails: string[] = []
    if (!addressOk) fails.push('address')
    if (!contactOk) fails.push('contactPhone')
    if (stripe !== true) fails.push('stripe')
    return { ref, name: String(r.businessName || ''), fails, hasBusinessPhone: !blank(a.phoneNumber) }
  })

  const atRisk = checked.filter(c => c.fails.length > 0)
  return NextResponse.json({ chunk: { start, count }, totalPages, orderingOnInChunk: on.length, atRiskCount: atRisk.length, atRisk })
}
